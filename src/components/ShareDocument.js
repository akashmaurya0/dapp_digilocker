...
//             const filteredRequests = await Promise.all(ownerRequests.map(async (request) => {
//                 try {
//                     // Check if the document exists by fetching its owner
//                     const docOwner = await contract.methods.getDocumentOwner(request.docId).call();
//                     return docOwner !== "0x0000000000000000000000000000000000000000" ? request : null;
//                 } catch (error) {
...
//             setRequests(filteredRequests.filter(request => request !== null));
//         } catch (error) {
...
//                                 <tr key={index}>
//                                     <td>{request.docId}</td> {/* Ensure docId is properly displayed */}
